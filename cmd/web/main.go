...
func main() {
	...
	// txnH is nil here because main.go uses in-memory stores without SQLite.
	// To enable the transaction module, open a sqlite.Open DB and wire
	// service/transaction.New with the SQLite-backed repo stores.
	router := web.NewRouter(authHandlers, sessionStore, auditSink, nil)
